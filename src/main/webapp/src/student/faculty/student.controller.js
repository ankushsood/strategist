...
'use strict';
	...
	function ViewStudentDetailsController($stateParams, GetStudentDetailsService, ModalService, $location, CreateStudentTimelineEventService){
		...
		_this.chartColors = ["#31C0BE", "#c7254e", "#98a0d3"];
		_this.myFormatter = function(input) {
			return input + '%';
		};
	...
	var accordianHide = function(showAccordian){
		if(!$('.panel div#' + showAccordian).is(":visible")){
			$('.panel div#collapseOne').slideUp();
			$('.panel div#collapseTwo').slideUp();
			$('.panel div#collapseThree').slideUp();
